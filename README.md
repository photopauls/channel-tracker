...
6. If any video *newly* crossed the outlier line since the last run, batches
   ...
   with the new outliers and that pattern digest. This Claude call can be
   switched off any time without touching code — see "Turning the Claude
   digest on/off" below — the Telegram alert with the plain outlier list
   still goes out either way, just without the pattern digest paragraph.
...
outliers show up, batched with the Telegram alert) runs automatically every
time the tracker runs, on every scheduled 6-hourly run, as long as
`ANTHROPIC_API_KEY` is set. There's no separate on/off switch for it out of
the box, but you don't need to touch code or remove the API key to turn it
off temporarily: in the repo, go to Settings → Secrets and variables →
Actions → **Variables** tab (not Secrets — this one isn't sensitive) → New
repository variable → name it `CLAUDE_DIGEST_ENABLED`, value `false`. The
very next tracker run will skip the Claude call entirely (you'll see
"Skipping Claude digest: CLAUDE_DIGEST_ENABLED is set to false" in the run
log) — outlier detection, the dashboard, and the plain Telegram alert
listing new outliers all keep working exactly the same, it's just the
Claude-generated pattern paragraph that stops. Delete the variable (or set
it to anything other than `false`) to turn it back on — no code change,
no redeploy, takes effect on the next run either way.
